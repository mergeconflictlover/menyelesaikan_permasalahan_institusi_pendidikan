...
- Prioritas 2: Gabungkan intervensi akademik dan finansial untuk siswa dengan `Tuition_fees_up_to_date = 0`. Dashboard menunjukkan `457` kasus dropout dari `528` siswa pada kelompok ini atau sekitar `86,6%`, sehingga institusi dapat membuat daftar pantauan harian lintas akademik-keuangan, menawarkan skema cicilan atau bantuan darurat, lalu memeriksa kembali progres akademik mereka pada bulan yang sama.
- Prioritas 3: Fokuskan program retensi pada siswa usia masuk `25 tahun ke atas`, terutama kelompok `25-29` dengan dropout rate `57,6%` dan kelompok `30+` dengan dropout rate `54,1%`. Implementasi praktisnya dapat berupa mentoring singkat, jadwal konsultasi di luar jam kerja, kelas malam, atau kanal dukungan daring yang lebih fleksibel untuk segmen nontradisional ini.
...
- Prioritas 5: Perlakukan status `Enrolled` sebagai kelompok transisi yang perlu evaluasi berkala, bukan kelompok aman. Karena kelas ini paling sulit diprediksi, siswa yang masih `Enrolled` tetapi memiliki approved units semester awal rendah atau mulai mendekati pola `Dropout` perlu ditinjau ulang pada setiap akhir semester menggunakan dashboard dan prototype machine learning.
